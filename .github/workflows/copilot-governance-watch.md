...
3. The remote GitHub MCP tools, which can search public source content without shell network access. Use this fallback only after the first two methods fail:
    ...
    - For a Blog or changelog entry, use `search_repositories` scoped to the `github` organization to identify a public repository hosting the entry. If there are multiple candidates, select only one whose repository metadata identifies it as an official GitHub Blog/changelog source and whose contents include the post title or URL slug. Then use `search_code` scoped to that repository and the post title.
